...
"""注意: session 必须在 cors 之前添加"""

"""给应用添加session中间件,使用这个中间件后, 接口就能用request.session保存和获取session了"""
# 添加中间件, 密钥, 过期时间 (SessionMiddleware 必须在 CORS 之前添加)
app.add_middleware(SessionMiddleware, secret_key=settings.session_secret_key, max_age=300) # max_age 设置会话过期时间
